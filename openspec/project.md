...
Cursor Agent is an OpenClaw plugin that invokes the local Cursor Agent CLI from OpenClaw chat conversations for deep code analysis, troubleshooting, and diagnostics. It automatically loads project context from `.cursor/rules`, `AGENTS.md`, etc., and supports enabling project-configured MCP servers (GitLab, databases, monitoring, etc.), providing users with full-stack code analysis capabilities.
...
- **External Dependency**: Cursor Agent CLI (locally installed `agent` command)
...
- Source organization:
  - `src/index.ts` — Plugin entry, registers `cursor_agent` tool
  - `src/types.ts` — Type definitions
  - `src/parser.ts` — Cursor Agent stream-json output parser
...
- **Cursor Agent CLI**: Command-line tool from Cursor IDE for AI-assisted code analysis
...
- Requires locally installed Cursor Agent CLI
...
- **Cursor Agent CLI**: Core dependency, installed via `curl https://cursor.com/install`
